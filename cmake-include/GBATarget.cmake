...
function(gba_target_link_runtime target library)
    ...
    elseif("${library}" STREQUAL "flashcart")
        ...
        if(ARGN)
            ...
            list(GET ARGN 1 reservedspace)

            message(STATUS "Save-type set to \"${savetype}\" (${reservedspace} bytes reserved)")
            message(STATUS "GbaFix Game Code \"U\" should be set to ${saveid}")
            target_compile_definitions(${target} PRIVATE "__gba_save_string=\"${savetype}\"")
            target_compile_definitions(flashcart PRIVATE "__gba_save_string=\"${savetype}\"")
            target_compile_definitions(${target} PRIVATE "__gba_save_start=${reservedspace}")
            target_compile_definitions(flashcart PRIVATE "__gba_save_start=${reservedspace}")
            target_compile_definitions(${target} PRIVATE "__gba_save_id=${saveid}")
            target_compile_definitions(flashcart PRIVATE "__gba_save_id=${saveid}")
        else()
            message(STATUS "Save-type set to \"none\" (0 bytes reserved)")
            target_compile_definitions(${target} PRIVATE "__gba_save_string=\"\"")
            target_compile_definitions(flashcart PRIVATE "__gba_save_string=\"\"")
            target_compile_definitions(${target} PRIVATE "__gba_save_start=0")
            target_compile_definitions(flashcart PRIVATE "__gba_save_start=0")
            target_compile_definitions(${target} PRIVATE "__gba_save_id=0")
            target_compile_definitions(flashcart PRIVATE "__gba_save_id=0")
        endif()
    else()
